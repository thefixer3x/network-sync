...
export interface HashtagAnalysis {
  ...
  difficultyScore: number; // How competitive it is
...
export class HashtagResearcher {
  private logger = new Logger('HashtagResearcher');
  private aiService = new OpenAIService();
  private cache = new Map<string, HashtagAnalysis>();
  private cacheExpiry = 6 * 60 * 60 * 1000; // 6 hours

  async researchHashtags(request: HashtagResearchRequest): Promise<HashtagStrategy> {
    try {
  ...
  private async getInstagramTrendingHashtags(): Promise<string[]> {
  ...
  private async generateAIHashtags(request: HashtagResearchRequest): Promise<string[]> {
...
Return only hashtags with # symbol, one per line:`;
  ...
  private async getRelatedHashtags(topic: string, platform: SocialPlatform): Promise<string[]> {
...
Return only hashtags with # symbol, one per line:`;
  ...
  private async getInstagramHashtagMetrics(hashtag: string): Promise<any> {
  ...
  private getEstimatedMetrics(hashtag: string, platform: SocialPlatform): any {
  ...
  private async getTopPostsForHashtag(hashtag: string, platform: SocialPlatform): Promise<any[]> {
  ...
  private getPeakTimes(platform: SocialPlatform): string[] {
  ...
  private generateBrandedHashtags(topic: string): string[] {

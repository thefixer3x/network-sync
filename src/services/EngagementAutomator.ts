...
import { Content, SocialMediaError } from '../types/typescript-types';
import { Logger } from '@/utils/Logger';
import type { PlatformCredentials } from './SocialMediaFactory';
...
export class EngagementAutomator {
  ...
  async addEngagementRule(
    rule: Omit<EngagementRule, 'id' | 'createdAt' | 'updatedAt'>
  ...
  private async findTwitterOpportunities(
  ...
  private async findLinkedInOpportunities(
    rules: EngagementRule[]
  ): Promise<EngagementOpportunity[]> {
  ...
  private matchesRuleConditions(rule: EngagementRule, tweet: any, author: any): boolean {
    ...
    if (
      rule.conditions.followerCountMin &&
      author.public_metrics.followers_count < rule.conditions.followerCountMin
    ) {
      return false;
    }

    if (
      rule.conditions.followerCountMax &&
      author.public_metrics.followers_count > rule.conditions.followerCountMax
    ) {
      return false;
    }
  ...
  async executeEngagementAction(
    ...
    try {
      ...
      if (success) {
        ...
        this.logger.success(
          `Executed ${action} on ${opportunity.platform} post ${opportunity.postId}`
        );
      }
  ...
  private async performLike(opportunity: EngagementOpportunity): Promise<boolean> {
    if (opportunity.platform === 'twitter') {
      ...
      try {
        await twitterClient.v2.like(
          (await twitterClient.currentUserV2()).data.id,
          opportunity.postId
        );
        return true;
      } catch (error) {
  ...
  private async performRetweet(opportunity: EngagementOpportunity): Promise<boolean> {
    if (opportunity.platform === 'twitter') {
      const twitterClient = this.socialServices.get('twitter') as TwitterApi;
      try {
        await twitterClient.v2.retweet(
          (await twitterClient.currentUserV2()).data.id,
          opportunity.postId
        );
        return true;
...
Generate only the reply text, ready to post:`;
  ...
  private canPerformAction(platform: SocialPlatform, action: string): boolean {
    ...
    return hourlyCount < (hourlyLimits[action] || 10) && dailyCount < (dailyLimits[action] || 100);
  }

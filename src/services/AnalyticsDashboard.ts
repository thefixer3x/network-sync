...
import { AccountMetrics, SocialPlatform, CompetitorAnalysis, Content } from '@/types';
...
export class AnalyticsDashboard {
  ...
  private async getPlatformPerformanceData(): Promise<any> {
    try {
      ...
      const result = Object.entries(platformData).map(([platform, data]: [string, any]) => ({
        ...
        followers: Math.round(data.followers.reduce((a: number, b: number) => a + b, 0) / data.followers.length),
        engagement: Number((data.engagement.reduce((a: number, b: number) => a + b, 0) / data.engagement.length).toFixed(2)),
        posts: Math.round(data.posts.reduce((a: number, b: number) => a + b, 0) / data.posts.length)
      }));
  ...
  async generateReport(type: 'daily' | 'weekly' | 'monthly', platforms?: SocialPlatform[]): Promise<DashboardReport> {
    try {
  ...
  private calculateGrowthRate(metrics: any[]): number {
    ...
    if (first.followers_count === 0) return 0;
...
Write in a professional tone suitable for executives and stakeholders.`;
  ...
  private async generateRecommendations(performanceMetrics: any, contentAnalysis: any): Promise<string[]> {
...
Return as a JSON array of recommendation strings:`;
  ...
  private async identifyGrowthOpportunities(performanceMetrics: any, competitorInsights: any): Promise<string[]> {

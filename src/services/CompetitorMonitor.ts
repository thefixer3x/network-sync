...
import { CompetitorAnalysis } from '../types/typescript-types';
...
export class CompetitorMonitor {
  ...
  private async analyzePlatform(
    ...
    switch (platform) {
      case 'twitter':
        return this.analyzeTwitterCompetitor(competitor, handle);
      case 'linkedin':
        return this.analyzeLinkedInCompetitor(competitor, handle);
      case 'instagram':
        return this.analyzeInstagramCompetitor(competitor, handle);
      default:
  ...
  private extractThemes(texts: string[]): string[] {
    ...
    commonWords.forEach((word) => {
      ...
      if (count > texts.length * 0.1) {
        // Appears in >10% of tweets
        themes.push(word);
      }
    });
  ...
  private async analyzeLinkedInCompetitor(
    competitor: CompetitorProfile,
    handle: string
  ): Promise<CompetitorInsight> {
    ...
    return {
      ...
      insights: {
        ...
        engagement: {
          averageRate: 2.5,
          topPerformingTypes: ['article'],
          audienceResponse: ['professional'],
        },
  ...
  private async analyzeInstagramCompetitor(
    ...
    return {
      ...
      insights: {
        ...
        engagement: {
          averageRate: 4.2,
          topPerformingTypes: ['video'],
          audienceResponse: ['visual_focused'],
        },
...
Return as JSON array of recommendation strings:`;
    ...
    try {
      ...
      return [
        'Analyze competitor posting times',
        'Study their content themes',
  ...
  private async generateAlerts(
    ...
    for (const [platform, insight] of Object.entries(insights)) {
      ...
      if (
        thresholds.followerGrowth &&
        insight.insights.growth.followerGrowthRate > thresholds.followerGrowth
      ) {

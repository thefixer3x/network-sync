...
import { SocialGrowthEngine, SocialGrowthConfig } from '../orchestrator/SocialGrowthEngine';
...
async function demonstrateCompleteWorkflow() {
  ...
  try {
    ...
    alertsAndRecommendations.recommendations.slice(0, 3).forEach((rec: string, i: number) => {
      logger.info(`  ${i + 1}. ${rec}`);
    });
...
async function demonstrateSpecificWorkflows(growthEngine: SocialGrowthEngine) {

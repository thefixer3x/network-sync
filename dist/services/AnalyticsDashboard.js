...
export class AnalyticsDashboard {
    ...
    async getTotalFollowersData() {
        try {
            ...
            const { data: previousMetrics } = await this.supabase
                ...
                .order('recorded_at', { ascending: false })
                .limit(10);
            const currentTotal = currentMetrics?.reduce((sum, metric) => sum + metric.followers_count, 0) || 0;
            const previousTotal = previousMetrics?.reduce((sum, metric) => sum + metric.followers_count, 0) || 0;
        ...
        try {
            ...
            while (currentWeek <= endDate) {
                ...
                if (weekMetrics.length > 0) {
                    ...
                    const avgEngagement = weekMetrics.reduce((sum, m) => sum + m.engagement_rate, 0) / weekMetrics.length;
                    weeklyData.push({
    ...
    async getTopPerformingPostsData() {
        try {
    ...
    async getPlatformPerformanceData() {
        try {
            const { data: metrics } = await this.supabase
                .from('account_metrics')
                .select('*')
            ...
            const result = Object.entries(platformData).map(([platform, data]) => ({
                platform: platform.charAt(0).toUpperCase() + platform.slice(1),
                followers: Math.round(data.followers.reduce((a, b) => a + b, 0) / data.followers.length),
                engagement: Number((data.engagement.reduce((a, b) => a + b, 0) / data.engagement.length).toFixed(2)),
                posts: Math.round(data.posts.reduce((a, b) => a + b, 0) / data.posts.length)
            }));
            return result;
        }
        catch (error) {
    ...
    async generateReport(type, platforms) {
        try {
            ...
            const recommendations = await this.generateRecommendations(performanceMetrics, contentAnalysis);
            const growthOpportunities = await this.identifyGrowthOpportunities(performanceMetrics, competitorInsights);
            const report = {
    ...
    calculateGrowthRate(metrics) {
        ...
        if (first.followers_count === 0)
            return 0;
        return Number((((last.followers_count - first.followers_count) / first.followers_count) * 100).toFixed(2));
    }
    async getContentAnalysis(filter) {
        const { data: content } = await this.supabase
            .from('content')
            .select('*')
        ...
        const byPlatform = published.reduce((acc, c) => {
        ...
        return {
            ...
            ai_generated_percentage: Math.round((content.filter(c => c.ai_generated).length / content.length) * 100),
            avg_posting_frequency: published.length / 7 // posts per week
        };
    }
    async getCompetitorInsights(filter) {
        ...
        const topCompetitor = analyses.reduce((top, current) => current.engagement_rate > top.engagement_rate ? current : top);
        return {
    ...
    async getEngagementTrendsChart(filter) {
        const { data: metrics } = await this.supabase
            .from('account_metrics')
    ...
    async getFollowerGrowthChart(filter) {
        const { data: metrics } = await this.supabase
            .from('account_metrics')
    ...
    async getContentPerformanceChart(filter) {
        const { data: content } = await this.supabase
            .from('content')
            .select('*')
            .eq('status', 'published')
    ...
    async getPlatformComparisonChart(filter) {
...
Write in a professional tone suitable for executives and stakeholders.`;
    ...
    async generateRecommendations(performanceMetrics, contentAnalysis) {
...
Return as a JSON array of recommendation strings:`;
    ...
    async identifyGrowthOpportunities(performanceMetrics, competitorInsights) {
...
Return as a JSON array of opportunity strings:`;
    ...
    async refreshAllWidgets() {
        const promises = Array.from(this.widgets.keys()).map(widgetId => this.refreshWidget(widgetId));

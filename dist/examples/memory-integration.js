...
export class EnhancedContentStorage {
    ...
    async storeMultiModalContent(params) {
        ...
        if (['jpg', 'jpeg', 'png', 'gif', 'webp'].includes(fileExtension || '')) {
            ...
            memoryResult = await memory.createImageMemory(`${platform} Image: ${filename}`, file, { extractText: true, generateDescription: true });
        }
            ...
            memoryResult = await memory.createDocumentMemory(`${platform} Document: ${filename}`, file, fileExtension);
        }
     ...
     */
    async getContentContext(topic, platform) {
    ...
    // Helper methods
    combineSearchResults(localResults, memoryResults) {
        // Combine and deduplicate results based on content similarity
        const combined = [...localResults];
        for (const memoryResult of memoryResults) {
            const isDuplicate = combined.some(local => this.calculateSimilarity(local.content, memoryResult.content) > 0.8);
            if (!isDuplicate) {
                combined.push({
                    ...memoryResult,
                    source: 'memory'
    ...
    generateContentRecommendations(memoryContext, trendingTopics) {
        return {
    ...
    async getHighPerformingContent(params) {
